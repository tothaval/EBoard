...
public partial class BrushSetupViewModel : ObservableObject
{
    ...
    public BrushSetupViewModel(FluidUIBaseViewModel eboardFluidUIBaseViewModel, BrushTargets brushTargets, Action okAction)
    {
        this.viewModel = eboardFluidUIBaseViewModel;
        this.fluidUIDesignManager = new FluidUIDesignManager(this.ViewModel);

        this.solidBrushViewModel = new SolidBrushSetupViewModel(eboardFluidUIBaseViewModel, brushTargets, this.ChangeToSolidColorBrush);

        this.linearBrushViewModel = new LinearBrushSetupViewModel(eboardFluidUIBaseViewModel, brushTargets, this);

        this.radialBrushViewModel = new RadialBrushSetupViewModel(eboardFluidUIBaseViewModel, brushTargets, this);

        this.okAction = okAction;

        this.brushTargets = brushTargets;

        this.brush = this.fluidUIDesignManager.GetBrush(brushTargets);
        this.imagePath = this.fluidUIDesignManager.GetImagePath(brushTargets);
    ...
    public Brush? Brush => this.brush;

    private VisualBrush TestMethodGetVisualBrush(int examplenr)
    {
    ...
    private void SetImageBrush(string path = "")
    {
        ...
        this.SetBrush(FluidUIDesignDefaultPropertyFactory.GetImageBrush(path));
    }
    ...
    [RelayCommand]
    private void ExampleVisualBrush(object? parameter)
    {
        if (parameter != null && parameter is string exampleNr)
        {
            var resultsucess = int.TryParse(exampleNr, out int nr);

            if (resultsucess)
            {
                this.SetBrush(this.TestMethodGetVisualBrush(nr));
            }
        }
    }
    ...
    {
        this.fluidUIDesignManager.ResetBrush(this.brushTargets);
        this.SetBrush(this.fluidUIDesignManager.GetBrush(this.brushTargets));
    }
